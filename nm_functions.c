    ...
    else if (strncmp(input, "write", strlen("write")) == 0)
    {
        ...
        if (node_in_cache == NULL)
        {
            ...
            else
            {
                ...
                // Insert the file into the LRU cache since it is recently used
                lru_node *new_lru_node = make_lru_node(file_path, ss_num, port, array_of_ss_info[ss_num].ss_ip);
        ...
        else
        {
            ...
            shift_node_to_front(file_path, head);
        }
    }
